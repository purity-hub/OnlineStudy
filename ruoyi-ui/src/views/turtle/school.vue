<template>
  <div class="app-container">
    <div class="hero is-small is-white">
      ...
      <div class="container">
        <div class="tab elastic-tab relative flex-wrap">
          <a href="/turtle/latest" class="tab-item border-b-2 border-transparent">
            <span>所有作品</span>
          </a>
          <a href="/turtle/upvotes" class="tab-item border-b-2 border-transparent">
            <span>最多点赞</span>
          </a>
          <a href="/turtle/colleges"
    ...
    <div class="box has-content-center has-text-centered">
      <br><br>
      <div class="text-2xl text-gray-800 font-bold mb-4">
        <b>{{collegeName}}</b> 作品库
      </div>
      <div class="has-content-center help">
        <b>指导老师：</b>
        <span v-for="(item,index) in teacherName">
          <a class="is-text-info" v-if="index===0">{{item}}</a>
          <a class="is-text-info" v-else>，{{item}}</a>
        </span>
      </div>
    </div>
...
import {getTurtleBySchool,getTeacherBySchool,updatelookNum,updatelikeNum} from "@/api/turtle/turtle";
...
export default {
  ...
  created() {
    if(this.$route.params.collegeName){
      this.collegeName = this.$route.params.collegeName;
      getTurtleBySchool(this.collegeName).then(res => {
        this.list = res.rows;
      });
      this.getTeacher();
    }
  ...
  methods: {
    ...
    //获取老师
    getTeacher() {
      getTeacherBySchool(this.collegeName).then(res => {
